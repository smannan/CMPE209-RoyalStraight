...
class Player:
    ...
    def mainMenu(self):
        if self.inRound:
            ...
            try:
                questions = [inquirer.List('options', message=query, choices=[
                                           'Leave Game', 'Fold', 'Join Round'],), ]
    ...
    def roundMenu(self):
        gameInfo = "The current pot is: " + str(self.game.getPot()) + " the current bet is: " + str(
            self.game.getBet()) + " your cards are: " + str(self.hand[0]) + " " + str(self.hand[1])
        try:
            questions = [inquirer.List('options', message=gameInfo, choices=[
                                       'Bet/Raise', 'Check/Match', 'Fold'],), ]
            answer = inquirer.prompt(questions)
        except:
            questions = {
                'type': 'list',
    ...
    def bet(self):
        query = "How much would you like to bet? bet value needs to be greater than current bet of " + \
            str(self.game.getBet())
        try:
        ...
        if bet > self.game.getBet() and self.balance >= bet:
            ...
            print(
                "Your bet is lower than current Bet, Up the ante!! Or you have not more funds:(  ")
            self.bet(self)
    ...
    def check(self):
        ...
        if self.balance >= bet:
            ...
            print(
                "Sorry! You don't have enough money to check this round, good luck next round")
            self.inRound = False
    def fold(self):
        # do some game logic here
        self.inRound = False
